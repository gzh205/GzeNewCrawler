...
namespace BlogWebsite.DynamicCompiler
{
    ...
    public class CompilerCore
    {
        ...
        public static CompilerCore GetCompiler()
        {
            if (inst == null)
            {
        ...
        public void Lexer(string code)
        {
            ...
            for (int i = 0; i < tmpcode.Length; i++)
            {
                ...
                {
                    Element tmpEle;
                    if (tmptok[k].Contains("(") && tmptok[k].Contains(")"))
                    {//是函数
                        tmpEle = new FunctionElement(tmptok[k], Element.Function);
                        restok.Add(tmpEle);
                    }
                    else if (tmptok[k].Contains("[") && tmptok[k].Contains("]"))
                    {//是数组
                        tmpEle = new Element(tmptok[k], Element.Array);
                        restok.Add(tmpEle);
                    }
                    else if (tmptok[k][0] >= '0' && tmptok[k][0] <= '9')
                    {//是数字
                        tmpEle = new Element(tmptok[k], Element.Number);
                        restok.Add(tmpEle);
                    }
                    else if (tmptok[k][0] >= 'a' && tmptok[k][0] <= 'z' || tmptok[k][0] >= 'A' && tmptok[k][0] <= 'Z')
                    {//是一个变量
        ...
        public Function arrange()
        {
            ...
            {
                if (tokens[i].deepth < depth)
                {//根据制表符\t判断该条语句是否已经离开函数定义
                    try
                    {
                    ...
                    {
                        functionlist[tmp.functionname] = tmp;
                    }
                ...
                else
                {
                    ...
                    {
                        //如果开头的第一个单词不是function，那么就是一个普通的句子，将其加入main函数
                        main.sentences = CalcSentences(new List<Sentence>() { tokens[i] });
                    }
                }
            }
            ...
            if (tmp != null)
            {
                if (functionlist.ContainsKey(tmp.functionname))
                {
                    functionlist[tmp.functionname] = tmp;
                }
                else
        ...
        /// </summary>
        public static void loadSystemApi()
        {
            Type[] types = Assembly.GetExecutingAssembly().GetTypes();
            foreach (Type t in types)
            {
                if (t.FullName.Contains("BlogWebsite.DynamicCompiler.Core.Library"))
                {
