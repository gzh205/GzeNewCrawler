    ...
    public class FunctionElement : Element
    {
        ...
        private List<Element> construct(string code)
        {
            ...
            foreach (string data in toks)
            {
                ...
                {//是函数
                    tmpEle = new FunctionElement(data, Function);
                    restok.Add(tmpEle);
                }
                else if (data.Contains("[") && data.Contains("]"))
                {//是数组
                    tmpEle = new Element(data, Array);
                    restok.Add(tmpEle);
                }
                else if (data[0] >= '0' && data[0] <= '9')
                {//是数字
                    tmpEle = new Element(data, Number);
                    restok.Add(tmpEle);
                }
                else if (data[0] >= 'a' && data[0] <= 'z' || data[0] >= 'A' && data[0] <= 'Z')
                {//是一个变量
                    tmpEle = new Element(data, Variable);
                    restok.Add(tmpEle);
                }
                else if (data[0] == '"')
                {//是字符串
                    tmpEle = new Element(data, String);
                    restok.Add(tmpEle);
                }
                else
                {//是符号
                    ...
                    else
                    {
                        if (data == ")")
                        {
                            ...
                            {
                                restok.Add(new Element(elestack.Pop(), Symbol));
                            }
                            if (elestack.Count == 0)
                            {
